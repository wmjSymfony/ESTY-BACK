...
async function DB() {
    pool = await mysqlpromise.createPool({
...
async function handleData() {
    await DB();
    connectionpromise = await pool.getConnection();
    ...
    for(let i = 0;i < listingids.length;i++){
        ...
        if(temp.status == 200){
            if(temp.hasOwnProperty('data') && temp.data.hasOwnProperty('results')){
                ...
                reviews = reviews.concat(temp.data.results);
            }
        }
    }
    let result = await listingsController(listingids);
    if(result.status == 200){
        await connectionpromise.beginTransaction();//mysql事务
        try {
            await insertListing(result,reviews_nums,rating_values);//插入listing
            await insertListingData(result,reviews_nums,rating_values);//插入listing表现
            await insertReviews(reviews);//插入评价
            await connectionpromise.commit();
            return {status:200,data:'success'}
        }catch (e) {
            await connectionpromise.rollback();//事务回滚
            console.log('database error',e);
            return {status:-1,data:'database insert fail'}
        }finally {
...
async function insertListing(result,reviews_nums,rating_values){
    ...
    if(insertData.length > 0){
        ...
        // try {
            ...
            console.log('listing Row inserted:' + rows.affectedRows);
            return {status:200};
        // }catch (e) {
...
async function insertListingData(result,reviews_nums,rating_values) {
    ...
    if(insertData.length > 0){
        ...
        // try {
            const [rows, fields] = await connectionpromise.query(stmt, [insertData]);
            console.log('listingdata Row inserted:' + rows.affectedRows);
            return {status:200};
        // }catch (e) {
        //     await connectionpromise.rollback();
        //     console.log('error',e);
